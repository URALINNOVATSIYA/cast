...
> **Примечание:** Документация создана ИИ.
...
#### `ToArray[A any](value any) A`
Преобразует значение в массив типа `A`. При ошибке вызывает panic.

**Аргументы:**
- `value any` - значение для преобразования

**Возвращает:**
- `A` - преобразованный массив

**Поддерживаемые типы:**
- массивы - возвращаются как есть
- срезы и другие массивоподобные значения - элементы преобразуются в тип массива
- указатели и интерфейсы - распаковываются и преобразуются
- `nil` - возвращает нулевое значение массива

**Пример:**
```go
result := cast.ToArray[[3]int]([]int{1, 2, 3})
// result: [3]int{1, 2, 3}

result := cast.ToArray[[2]string]([]any{"a", "b"})
// result: [2]string{"a", "b"}
```

#### `AsArray[A any](value any) (A, error)`
Преобразует значение в массив типа `A`. Возвращает ошибку, если преобразование невозможно.

**Пример:**
```go
result, err := cast.AsArray[[3]int]([]string{"1", "2", "3"})
if err != nil {
    log.Fatal(err)
}
// result: [3]int{1, 2, 3}

_, err = cast.AsArray[int]("invalid")
// err: generic type must be an array type, int given
```
